...
export function ShellLayout({ children, onNavigate, activeRoute }: ShellLayoutProps) {
                ...
                {(node.children || [node]).map((child) => (
                  <li key={child.id}>
                    <button
                      ...
                      } ${activeRoute && child.route && activeRoute.startsWith(child.route) ? 'bg-muted' : ''}`}
      ...
      <main className="flex-1">
        <header className="border-b border-border px-6 py-4">
          <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div>
              <div className="text-xs uppercase text-muted-foreground">Serp Desktop Shell</div>
